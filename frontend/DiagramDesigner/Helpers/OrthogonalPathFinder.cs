...
namespace DiagramDesigner
{
    ...
    public class OrthogonalPathFinder : IPathFinder
    {
        ...
        private static List<Point> OptimizeLinePoints(List<Point> linePoints, Rect[] rectangles, ConnectorOrientation sourceOrientation, ConnectorOrientation sinkOrientation)
        {
            ...
            for (int j = 0; j < points.Count - 1; j++)
            {
                if (points[j].X != points[j + 1].X && points[j].Y != points[j + 1].Y)
                {
                    ...
                    if ((orientationFrom == ConnectorOrientation.TrueOutput || orientationFrom == ConnectorOrientation.FalseOutput) &&
                        (orientationTo == ConnectorOrientation.TrueOutput || orientationTo == ConnectorOrientation.FalseOutput))
                    {
        ...
        private static ConnectorOrientation GetOrientation(Point p1, Point p2)
        {
            ...
            {
                if (p1.Y >= p2.Y)
                    return ConnectorOrientation.FalseOutput;
                else
                    return ConnectorOrientation.TrueOutput;
            }
            else if (p1.Y == p2.Y)
            {
                if (p1.X >= p2.X)
